...
{
    struct Compiler *enclosing; // parent
...
{
    if (parser.panicMode)
        return; // we're already handling an error
...
static int emitJump(uint8_t instruction)
{
    ...
    // A placeholder, to be overwritten. It's a short, so two bytes.
    emitByte(0xff);
    emitByte(0xff);
    // Return the location to ovewrite.
...
{
    // Subtract 2 to account for the jump offset itself.
...
static void endScope()
{
    ...
    // Remove any leftover local variables.
...
static void dot(bool canAssign)
{
    ...
    {
        // fast path for method invocations: we know this is going to be called
        // immediately so we can avoid heap allocation.
        uint8_t argCount = argumentList();
...
static void this_(bool canAssign)
{
    ...
    // Treat 'this' as a lexically scoped local variable (which is automagically
...
static void addLocal(Token name)
{
    ...
    Local *local = &current->locals[current->localCount++];
    local->name = name;
    local->depth = -1; // uninitialized for now
    local->isCaptured = false;
}
...
static void ifStatement()
{
    ...
    emitByte(OP_POP); // clear the condition result
    statement();
    ...
    if (!check(TOKEN_RIGHT_PAREN))
    {
        ...
        do
        {
            ...
            if (current->function->arity > 255)
                errorAtCurrent("Function can not have more than 255 parameters.");
...
static void forStatement()
{
    ...
    // The (optional) loop termination clause.
    ...
    if (!match(TOKEN_SEMICOLON))
    {
        ...
        // Jump out of the loop if the condition no longer holds.
        exitJump = emitJump(OP_JUMP_IF_FALSE);
        emitByte(OP_POP);
    }

    // The increment clause. It's a bit odd… we jump over the increment, run the
    // body then jump back to the increment. But this means we can operate in a
    // single pass!
