...
void initVM()
{
    ...
    // GC: ensure we don't see an uninitialized value
    vm.initString = NULL;
...
static bool callValue(Value callee, int argCount)
{
    if (IS_OBJ(callee))
    {
        ...
        case OBJ_CLASS:
        {
            ...
            {
                // The arguments are still on the stack, so we can pass them
...
static ObjUpvalue *captureUpvalue(Value *local)
{
    ...
    // As open upvalues are stored in stack slot order, we can know when we are
    // past.
...
static void concatenate()
{
    ...
    // Remove the originals, which had to be left on the stack for GC
    // reachability.
    pop();
    pop();
    push(OBJ_VAL(result));
